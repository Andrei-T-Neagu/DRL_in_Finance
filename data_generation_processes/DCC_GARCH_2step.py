...
class DCC_GARCH():
    ...
    def __init__(self, data):
        ...
        self.Q = np.copy(self.Q_bar)                                                         #(N, N)
        self.params_garch = np.zeros(self.N*3)+0.005                                     #[N*alpha0, N*alpha, N*beta, a, b]
        self.params_dcc = np.zeros(2)+0.005
        ...
        # self.nll_losses = []
        # self.prev_params = np.copy(self.params)
        # self.prev_nll = 0
        # self.params = [1.04384412e-05, 5.56351037e-06, 3.14464258e-01, 3.24972411e-01, 7.64612517e-05, 1.00717994e-03, 2.27767312e-01, 1.10808117e-04]
        def constraint_ab(x):
            return 1.0 - x[-2] - x[-1]
        ...
        self.constraints_garch = [{'type':'ineq', 'fun':constraint_alpha_beta}]
        self.constraints_dcc = [{'type':'ineq', 'fun':constraint_ab}]

        self.bounds_garch = Bounds([np.finfo(float).eps for i in range(self.N*3)], [np.inf for i in range(self.N*3)], keep_feasible=[True for i in range(self.N*3)])
        self.bounds_dcc = Bounds([np.finfo(float).eps for i in range(2)], [np.inf for i in range(2)], keep_feasible=[True for i in range(2)])
    ...
    def neg_log_likelihood_garch(self, params):
        # if np.any(params < 0.0) or np.any(params > 1.0) or np.any(params[-2] + params[-1] > 1.0) or np.any(params[self.N:2*self.N] + params[2*self.N:3*self.N] > 1.0):
        #     params = np.copy(self.prev_params)
        #     return self.prev_nll+np.abs(self.prev_nll)*0.001
        print("params: ", params)
        train = True
        log_likelihood = 0
        for t in range(self.T):
            self.D_t(params=params, t=t, train=train)
            current_log_likelihood = self.N * np.log(2*np.pi) + 2*np.log(np.linalg.det(self.D)) + self.a_return.T@np.linalg.inv(self.D)@np.identity(self.N)@np.linalg.inv(self.D)@self.a_return
            log_likelihood += current_log_likelihood
        nll = log_likelihood.flatten()[0]/2
        print("nll: ", nll)
        print()
        # self.nll_losses.append(nll)
        # self.prev_params = np.copy(params)
        # self.prev_nll = np.copy(nll)
        return nll

    def neg_log_likelihood_dcc(self, params):
        # if np.any(params < 0.0) or np.any(params > 1.0) or np.any(params[-2] + params[-1] > 1.0) or np.any(params[self.N:2*self.N] + params[2*self.N:3*self.N] > 1.0):
        #     params = np.copy(self.prev_params)
        #     return self.prev_nll+np.abs(self.prev_nll)*0.001
        print("params: ", params)
        ...
        log_likelihood = 0
        for t in range(self.T):
            self.D_t(params=self.params_garch, t=t, train=train)
    ...
    def train(self):
        params_garch = self.params_garch
        results_garch = minimize(fun=self.neg_log_likelihood_garch, x0=params_garch, method="slsqp",
                           options={'disp': True}, constraints=self.constraints_garch, bounds=self.bounds_garch)
        self.params_garch = results_garch.x
        params_dcc = self.params_dcc
        results_dcc = minimize(fun=self.neg_log_likelihood_dcc, x0=params_dcc, method="slsqp",
                           options={'disp': True}, constraints=self.constraints_dcc, bounds=self.bounds_dcc)
        self.params = results_dcc.x
        return results_garch.x, results_dcc.x

    def generate(self, S_0, num_points):
        prices = np.ones((self.N, num_points)) * S_0
        for t in range(num_points):
            self.D_t(params=self.params_garch)
            self.R_t(params=self.params_dcc)
            r_t = self.r_t()
...
params_garch, params_dcc = model.train()
