...
if garch_type == "gjr":
    ...
    # Step 1: Define the vanilla GARCH model specification
    garch_spec <- ugarchspec(variance.model = list(model = "sGARCH", garchOrder = c(1, 1)),
                            mean.model = list(armaOrder = c(0, 0)),
                            distribution.model = "norm")

    # Step 2: Fit the GARCH model to the returns data
    garch_fit <- ugarchfit(spec = garch_spec, data = returns_data)
