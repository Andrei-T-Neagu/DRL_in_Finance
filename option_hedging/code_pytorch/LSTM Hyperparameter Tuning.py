...
if light:
    ...
    name_lstm = 'code_pytorch/lstm_model_light'
else:
...
for i, config in enumerate(hyperparameters):
    ...
    if light:
        agent_lstm = DeepAgentLSTMLight.DeepAgent(nbs_point_traj, config["batch_size"], r_borrow, r_lend, stock_dyn, params_vect, S_0, T, alpha, beta,
    ...
    if light:
        agent_lstm = DeepAgentLSTM.DeepAgent(nbs_point_traj, config["batch_size"], r_borrow, r_lend, stock_dyn, params_vect, S_0, T, alpha, beta,
                    loss_type, option_type, position_type, strike, V_0, config["nbs_layers"], config["nbs_units"], lr, prepro_stock,
                    nbs_shares, lambdas, name=name_lstm)

    num_parameters.append(count_parameters(agent_lstm))
    ...
    deltas_lstm_valid, hedging_err_lstm_valid, S_t_lstm_valid, V_t_lstm_valid, A_t_lstm_valid, B_t_lstm_valid = agent_lstm.test(test_size=valid_size, test_set=valid_set_batched)

    deltas_lstm_test, hedging_err_lstm_test, S_t_lstm_test, V_t_lstm_test, A_t_lstm_test, B_t_lstm_test = agent_lstm.test(test_size=test_size, test_set=test_set_batched)
    if loss_type == "RSMSE":
        semi_square_hedging_err_lstm_valid = np.square(np.where(hedging_err_lstm_valid > 0, hedging_err_lstm_valid, 0))
        semi_square_hedging_err_lstm_test = np.square(np.where(hedging_err_lstm_test > 0, hedging_err_lstm_test, 0))
        rsmse_lstm_valid = np.sqrt(np.mean(semi_square_hedging_err_lstm_valid))
        rsmse_lstm_test = np.sqrt(np.mean(semi_square_hedging_err_lstm_test))
        error_valid = rsmse_lstm_valid
        error_test = rsmse_lstm_test
    else: 
        rmse_lstm_valid = np.sqrt(np.mean(hedging_err_lstm_valid))
        rmse_lstm_test = np.sqrt(np.mean(hedging_err_lstm_test))
        error_valid = rmse_lstm_valid
        error_test = rmse_lstm_test
    
    config_valid_losses.append(error_valid)
    config_test_losses.append(error_test)
    config_losses_per_epoch.append(lstm_losses)

sorted_valid_indices = np.argsort(config_valid_losses).tolist()

with open(path_prefix + "lstm_hyperparameter_tuning.txt", "w") as hyperparameter_tune_file:
    # Writing data to a file
    hyperparameter_tune_file.write("|---------------------------Hyperparameter Tuning for LSTM for {epochs:d} Epochs------------------------|\n".format(epochs=epochs))
...
plt.xlabel("Epoch")
plt.ylabel("Error")
plt.title("Error per Epoch of Hyperparameter Search for LSTM Architecture")
plt.savefig(path_prefix + "lstm_hyperparameter_search.png")