...
class DeepAgent():
    ...
    def __init__(self, nbs_point_traj, batch_size, r_borrow, r_lend, stock_dyn, params_vect, S_0, T, alpha, beta,
        ...
        # Model
        self.model = LSTM_multilayer_cell(batch_size=self.batch_size, input_size=3, hidden_size=self.nbs_units, num_layers=self.nbs_layers, device=self.device, dropout=dropout).to(self.device)
